...
public abstract class AbstractUInt /*extends Number */ implements UInt {
    ...
    AbstractUInt(long uInt, Type uIntType) {
        ...
        if (!isValidUInt(uInt))
            throw new de.wiltherr.ws2812fx.serial.serialPacketCommunicator.exception.IllegalIntException(String.format("uInt was %d but only int from %d to %d can converted to unsigned %d bit integers.", uInt, MIN_VALUE, uIntType.maxValue(), uIntType.bitSize()));
    }
