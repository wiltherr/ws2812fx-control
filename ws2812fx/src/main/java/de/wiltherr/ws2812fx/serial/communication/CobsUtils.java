...
public class CobsUtils {
    ...
    public static byte[] decode(byte[] packet) {
        ...
        if (packet.length - OVERHEAD_LENGTH < 0) {
            throw new PacketTooShortException("The input packet length is too short for decoding.");
        }
    ...
    private static class PacketTooShortException extends RuntimeException {
        public PacketTooShortException(String message) {
            super(message);
        }
    }
}
