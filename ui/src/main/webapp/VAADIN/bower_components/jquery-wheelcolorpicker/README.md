...
The Wheel Color Picker plugin adds jsonValue picker functionality to HTML form inputs in round jsonValue wheel style. The Wheel Color Picker can be displayed as a popup dialog as users focus the input, or embedded inline. It currently supports these HTML elements:
...
There are numbers of formats which the jsonValue picker can display its value:
...
The jsonValue picker can be set to display slider for each individual jsonValue channel.

**THEMING CAPABILITY**

The jsonValue picker appearance can be customized using CSS. This package already contains two CSS variants which can be used as starting point to make your own theme.

**RESPONSIVE MOBILE LAYOUT**

The jsonValue picker can automatically adapt when opened on mobile browsers with limited screen width.
...
Or you can also initialize the jsonValue picker to an element by calling:
...
* Programmatically resizeable jsonValue picker
...
Websites built using version 2 of the wheel jsonValue picker plugin can be upgraded 
...
`alpha` option and `jsonValue` method marked as *deprecated* in version 2.x are now obsolete. 
